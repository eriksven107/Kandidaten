...
console.log("\nBefore sending SCT:"+(Date.now()-start)/1000;
  const request = new Request('http://localhost:1895/', {method: 'POST',headers:{'Content-Type': 'application/json'} ,body: JSON.stringify(data)});
 fetch(request);
 console.log("\nAfter sending SCT:"+(Date.now()-start)/1000);
}
};
...
browser.webRequest.onHeadersReceived.addListener(
  details => { consume(details); start=Date.now();
